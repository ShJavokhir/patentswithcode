#!/usr/bin/env python3
"""
SQS Message Processor with E2B Claude Code Integration.
Polls SQS queue and processes code generation tasks using Claude Code in E2B sandbox.
"""
import json
import os
import sys
...
from e2b_claude_handler import process_sqs_message_sync
...
class SQSClaudeE2BProcessor:
    """Processes SQS messages for code generation using Claude Code in E2B sandbox."""

    def __init__(self, queue_url: str, region_name: Optional[str] = None):
        """
        Initialize the SQS Claude E2B processor.
    ...
    def poll_and_process(self, max_messages: int = 1, wait_time: int = 20):
        """
        Poll messages from SQS and process them with Claude Code in E2B.

        Args:
            max_messages: Maximum number of messages to retrieve per request (1-10)
            wait_time: Long polling wait time in seconds (0-20)
        """
        logger.info(f"Starting SQS Claude E2B processor for queue: {self.queue_url}")
                ...
                for message in messages:
                    try:
                        ...
                        if result.get('status') == 'completed':
                            ...
                            logger.warning(f"Message processing failed, leaving in queue for retry")
    ...
    def _process_message(self, message: dict) -> Dict[str, Any]:
        """
        Process a single SQS message with Claude Code E2B handler.
        ...
        # Process with Claude Code E2B handler
        result = process_sqs_message_sync(body)

        # Add message metadata to result
        result['message_id'] = message.get('MessageId')
        result['receipt_handle'] = message.get('ReceiptHandle')[:50] + '...'  # Truncate for logging
    ...
    def _save_result(self, result: Dict[str, Any]):
        ...
        filename = f"{results_dir}/{task_id}_{timestamp}.json"

        # Remove receipt handle from saved data (it's too long and not useful)
        save_data = {k: v for k, v in result.items() if k != 'receipt_handle'}
        ...
        if result.get('status') == 'completed':
            ...
            if result.get('files_created'):
                ...
                if len(result['files_created']) > 5:
                    logger.info(f"    ... and {len(result['files_created']) - 5} more")
        else:
...
def main():
    """Main entry point for the SQS Claude E2B processor."""
    ...
    print("\n" + "="*60)
    print("  SQS Claude Code E2B Processor")
    print("="*60)
    print(f"  Region: {region_name}")
    print(f"  Queue: {queue_url}")
    print("  Using Claude Code CLI in E2B Sandbox")
    print("="*60 + "\n")

    try:
        processor = SQSClaudeE2BProcessor(queue_url=queue_url, region_name=region_name)
